...
class ProfileVC: UIViewController, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    var genderList = UserGender.sharedInstance().gender as! Array<String>
    ...
    @IBAction func updateBtn(_ sender: Any) {
        ...
        dob = dobTF.text!.isEmpty ? "" : FD.formatDateSQL(datePicker.date)
    ...
    @objc func dateChange(datePicker: UIDatePicker){
        dobTF.text = FD.formatDate(datePicker.date)
        dob = FD.formatDateSQL(datePicker.date)
    }
                    ...
                    if(endPoint == "logout"){
                        ...
                        if((json["check"])! as! Bool){
                            ...
                            DispatchQueue.main.async {
                                ...
                                self.dob = resultdob is String ? self.FD.formatDate(self.FD.date(fromISOString: resultdob as? String)) : ""
                                self.dobTF.text = self.dob
                                
                                self.datePicker.date = !self.dob.isEmpty ?
                                self.FD.date(forPV: self.dob) : Date()
