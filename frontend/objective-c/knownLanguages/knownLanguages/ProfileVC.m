...
//  knownLanguages
//
//  Created by Tan Junhe on 1/9/23.
//
            ...
            if(!error) {
                if([endPoint isEqualToString:@"logout"]){
                    ...
                    dispatch_async(dispatch_get_main_queue(), ^{
                        ...
                        self.dob = [[result0 objectForKey:@"DOB"] isKindOfClass:[NSString class]] ? [self.FD formatDate:[self.FD dateFromISOString:[result0 objectForKey:@"DOB"]]] : @"";
                    
                        [self.dobTF setText:self.dob];

                        self.datePicker.date = self.dob ? [self.FD dateForPV:[self.FD formatDate: [self.FD dateFromISOString:self.dob]]] : [NSDate date];
